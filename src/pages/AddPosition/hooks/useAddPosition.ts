...
export const useAddPosition = (isCreatePool: boolean, poolInfo?: Pool) => {
  ...
  const { reserve0, reserve1 } = useMemo(() => {
    if (isCreatePool) {
      ...
      if (BigInt(`0x${beginCell().storeAddress(routerJettonWallet0).endCell().hash().toString('hex')}`) <
        BigInt(`0x${beginCell().storeAddress(routerJettonWallet1).endCell().hash().toString('hex')}`)
      ) {
        ...
        const initPrice = Number(createPoolParams.initPrice);
        // console.log('initPrice', initPrice * 10 ^ (token1.decimals - token0.decimals));
        const { numerator, denominator } = toFraction(initPrice * 10 ** (token0.decimals - token1.decimals));
        console.log({
          reserve0: numerator,
          reserve1: denominator
        })
        return {
          reserve0: numerator,
          reserve1: denominator,
